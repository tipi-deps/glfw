...
if(NOT Python_FOUND)
    message( FATAL_ERROR "CMake could not find python on your path" )
endif()

# the build of libxcb fails for python > 3.8 because of a broken import statement
# in the build scripts. If Cmake finds a 3.9+ version of python we *try* to find another
# version before throwing an error
if(Python_VERSION_MAJOR GREATER_EQUAL 3 AND Python_VERSION_MINOR GREATER 8)

    # try to find a python2
    find_package(Python2 COMPONENTS Interpreter REQUIRED)

    if(NOT Python2_FOUND)
        message( FATAL_ERROR "We found a python version > 3.8.x on your system only. LibXcb has a dependency on an earlier version of python. Please install python2 to proceed.")
    else()
        set(ENV{PYTHON} Python2_EXECUTABLE)        
    endif()
